...
-    **Photos fix tools** — guided GPS/capture-date fix modes with suggestions, interpolation and bulk preview. See `features/30`. Null-island `(0,0)` is dropped at the read path (both engines + the received merge) and rejected on write, rather than the planned client heuristic — such pictures now read as plainly missing GPS. *Deferred:* date run-interpolation, batched-propose endpoint.
-    **GPS accuracy** — `gps_accuracy_m` write-through as EXIF `GPSHPositioningError`; the fix tools suggest it from the anchors' spread (Suggested / Exact / Same as photo), editable everywhere GPS is. Existing pictures are populated by a new `synced` scope of the admin EXIF recheck, which re-reads metadata without regenerating thumbnails and walks a keyset cursor. See `features/36_gps_accuracy.md`. *Deferred:* a rule field on the accuracy.
...
-    **Tag browse follow-ups** — `exact` (the *Direct only* view and `exact_count`) is now a **deepest-tag** match, so a picture carrying both a segmentation tag and a manual tag under it renders once (35 §2); subtag ordering gained a direction (`children_order_desc`, 34 §7); the metadata write queue commits a flushed batch into the served payload instead of dropping the overlay onto stale rows, which made the tree silently revert a minute after every edit (34 §4.1).
