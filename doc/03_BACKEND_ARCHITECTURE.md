...
| `POST` | `/api/authenticated/pictures/uploads/{picture_id}/complete` | Confirm upload. Optional body: `{ mime_type, file_size, width, height, initial_tags?, defer_pipeline?, ... }`. Assigns `initial_tags` as manual tags (validated), enqueues a `gen_thumbnail` job; all created atomically in one transaction. `defer_pipeline=true` skips the per-file pipeline wake. |
| `POST` | `/api/authenticated/pictures/pipeline/wake`                 | Explicitly wake the caller's tagging pipeline (once, after a batch upload that deferred per-file wakes).                                                                                                                                                                                             |
