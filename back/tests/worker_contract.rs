...
/// §4.6: a successful extraction settles the **read** direction. For a video — readable by ffprobe,
/// never writable — the write verdict is already known, so the completion stamps `unsupported_mime`
/// rather than `synced`, and the set-based batch path reads a complete verdict off the column.
/// `file_exif` still witnesses the successful read (§4.7), which is what separates this row from a
/// GIF's `unsupported_mime`.
#[sqlx::test(migrator = "MIGRATOR")]
async fn video_extraction_settles_on_the_write_verdict(db: PgPool) {
    let settings = test_settings_with(&[]);
    let token = worker_token(&settings);
    let alice_id = common::seed_user(&db, "alice", "pass").await;
    let pic_id = common::seed_picture(&db, alice_id).await;
    sqlx::query!(
        "UPDATE pictures SET mime_type = 'video/mp4', exif_sync_status = 'extracting' WHERE id = $1",
        pic_id,
    )
    .execute(&db)
    .await
    ...
    let resp = app
        ...
        .oneshot(post_json(
            ...
            &serde_json::json!({
                "claim_token": claim_token,
                "thumbnails_generated": true,
                "exif": {"extracted": {"gps_lat": 48.0, "gps_lng": 2.0}},
            }),
        ))
        .await
        .unwrap();
    ...
    assert_eq!(
        picture.exif_sync_status,
        ExifSyncStatus::UnsupportedMime,
        "readable, but the write direction is settled the moment the read lands",
    );
    assert_eq!(picture.gps_lat, Some(48.0), "the read still lands");
    assert!(
        picture.file_exif.is_some(),
        "the successful-read witness distinguishes this from a never-read unsupported_mime",
    );
}
