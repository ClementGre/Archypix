...
///
/// `instance` is the user's global domain. If it differs from `config.get(setting_keys::GLOBAL_DOMAIN)` the
...
#[tracing::instrument(skip(cache, db, settings))]
pub async fn find_local_user_id(
    ...
    if instance != settings.get(keys::GLOBAL_DOMAIN) {
        return Ok(None);
    }
    ...
    let _ = cache
        .set_str_ex(
            key,
            &value,
            settings.get(keys::FEDERATION_BACKEND_CACHE_TTL_SECS),
        )
        .await;
...
/// resolver-supplied value or from settings; `None`/`Some(0)` leaves the
